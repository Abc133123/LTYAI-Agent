...
class CustomWorld(mmdpy_world.world):
    ...
    def __init__(self, window_name: str, window_width: int, window_height: int):
        ...
        # 初始化Qwen模型
        self.qwen_llm = QwenLLM("qwen_lora_result_v3")
        ...
        self.loading_thread = threading.Thread(target=self.load_model_async)
    ...
    def load_model_async(self):
        """异步加载模型"""
        success = self.qwen_llm.load_model()
    ...
    def key_callback(self, window, key, scancode, action, mods):
        ...
        if action == glfw.PRESS:
            ...
            # Enter键发送消息
            if key == glfw.KEY_ENTER and self.chat_visible and not self.generating_response:
                self.send_message()
                return
            
            # Backspace键删除字符
            if key == glfw.KEY_BACKSPACE and self.chat_visible and not self.generating_response:
    ...
    def char_callback(self, window, char):
        """字符输入回调"""
        if self.chat_visible and not self.generating_response and char >= 32 and char <= 126:
            # 添加可打印字符到输入文本
            self.input_text += chr(char)
    
    def send_message(self):
        """发送消息"""
        if not self.input_text.strip() or self.generating_response or self.model_loading:
            return
        
        # 添加用户消息到聊天历史
        self.chat_messages.append(("你", self.input_text))
        ...
        generation_thread = threading.Thread(
            target=self.generate_response_async,
            args=(self.chat_messages[-1][1],)
        )
    ...
    def generate_response_async(self, user_input):
        """异步生成回复"""
        response = self.qwen_llm.generate_response(user_input)
    ...
    def draw_chat_interface(self):
        ...
        try:
            ...
            # 聊天界面尺寸和位置
            chat_width = min(600, self.window_width - 40)
            chat_height = min(400, self.window_height - 40)
            chat_x = 20
            chat_y = 20
            ...
            self.draw_rounded_rect(chat_x, chat_y, chat_width, chat_height, 10)
            
            # 标题栏
            title_height = 30
            gl.glColor4f(0.2, 0.2, 0.2, 0.9)
            self.draw_rounded_rect(chat_x, chat_y + chat_height - title_height, chat_width, title_height, 10, top_only=True)
            
            # 标题文本
            gl.glColor4f(1.0, 1.0, 1.0, 1.0)
            self.draw_text("洛天依AI聊天 (Tab键切换)", chat_x + 10, chat_y + chat_height - title_height + 5, 0.5)
            
            # 聊天历史区域
            history_y = chat_y + 10
            history_height = chat_height - title_height - 60
            
            # 绘制聊天历史背景
            gl.glColor4f(0.05, 0.05, 0.05, 0.7)
            self.draw_rounded_rect(chat_x + 10, history_y, chat_width - 20, history_height, 5)
            
            # 显示最近的聊天消息
            max_messages = min(10, len(self.chat_messages))
            start_idx = max(0, len(self.chat_messages) - max_messages)
            
            y_offset = history_y + history_height - 20
            for i in range(start_idx, len(self.chat_messages)):
                sender, message = self.chat_messages[i]
                
                # 发送者名称
                gl.glColor4f(0.3, 0.6, 1.0, 1.0)  # 蓝色
                self.draw_text(f"{sender}:", chat_x + 20, y_offset, 0.4)
                y_offset -= 20
                
                # 消息内容（自动换行）
                gl.glColor4f(1.0, 1.0, 1.0, 1.0)  # 白色
                lines = self.wrap_text(message, chat_width - 40)
                for line in lines:
                    self.draw_text(line, chat_x + 20, y_offset, 0.4)
                    y_offset -= 15
            ...
            if self.generating_response:
                gl.glColor4f(0.3, 0.6, 1.0, 1.0)  # 蓝色
                self.draw_text("洛天依:", chat_x + 20, y_offset, 0.4)
                y_offset -= 20
                
                gl.glColor4f(1.0, 1.0, 1.0, 1.0)  # 白色
                self.draw_text("正在思考中...", chat_x + 20, y_offset, 0.4)
            elif self.current_response and not self.generating_response:
                # 显示最新的回复
                gl.glColor4f(0.3, 0.6, 1.0, 1.0)  # 蓝色
                self.draw_text("洛天依:", chat_x + 20, y_offset, 0.4)
                y_offset -= 20
                ...
                gl.glColor4f(1.0, 1.0, 1.0, 1.0)  # 白色
                lines = self.wrap_text(self.current_response, chat_width - 40)
                for line in lines:
                    self.draw_text(line, chat_x + 20, y_offset, 0.4)
                    y_offset -= 15
            ...
            # 输入框背景
            gl.glColor4f(0.05, 0.05, 0.05, 0.7)
            self.draw_rounded_rect(chat_x + 10, input_y, chat_width - 20, input_height, 5)
            
            # 输入框文本
            gl.glColor4f(1.0, 1.0, 1.0, 1.0)
            input_display = self.input_text
            if self.cursor_visible and int(time.time() * 2) % 2 == 0:
                input_display += "_"
            self.draw_text(input_display, chat_x + 20, input_y + 10, 0.4)
            
            # 提示文本
            if self.model_loading:
                gl.glColor4f(1.0, 1.0, 0.0, 1.0)  # 黄色
                self.draw_text("正在加载模型...", chat_x + 20, input_y + 25, 0.4)
            else:
                gl.glColor4f(0.7, 0.7, 0.7, 1.0)  # 灰色
                self.draw_text("输入消息，按Enter发送", chat_x + 20, input_y + 25, 0.4)
    ...
    def draw_rounded_rect(self, x, y, width, height, radius, top_only=False):
        """绘制圆角矩形"""
        # 简化实现：只绘制普通矩形
        gl.glBegin(gl.GL_QUADS)
        gl.glVertex2f(x, y)
    ...
    def draw_text(self, text, x, y, scale):
        """绘制文本"""
        # 使用OpenGL的位图字体绘制文本
        ...
        for char in text:
            ...
            try:
                # 尝试使用GLUT字体
                if hasattr(glut, 'glutBitmapCharacter'):
    ...
    def wrap_text(self, text, max_width):
        """将文本按最大宽度换行"""
        # 简化实现：假设每个字符宽度为10像素
        chars_per_line = int(max_width / 10)
        if chars_per_line <= 0:
            return [text]
...
# 保留原有的HairPhysicsSimulator类和其他函数
class HairPhysicsSimulator:
    """简单的麻花辫物理模拟器"""
    def __init__(self):
        self.gravity = np.array([0, 25.0, 0]) * 0.02  # 大幅增加重力，使麻花辫极端向下
        self.damping = 0.85  # 减少阻尼，使动作更自然
    ...
    def init_hair_segments(self, hair_bones):
        ...
        for i, bone in enumerate(sorted(left_bones, key=lambda b: int(b.name.split('_')[2]) if '_' in b.name and len(b.name.split('_')) >= 3 else 0)):
            ...
            init_x = -5.0  # 左侧，极端远离头部
            init_y = 15.0 + i * 1.2  # 从更高位置开始，每段增加1.2（更极端的向下）
            init_z = 3.0  # 更向前，远离眼睛
    ...
    def update(self, dt=0.016):
        ...
        for i, segment in enumerate(self.segments):
            ...
            if i > 0:
                ...
                if segment['is_left']:
                    # 左侧固定点 - 调整到更自然的头部侧面位置
                    segment['position'][0] = -5.0  # 极端远离头部
                    segment['position'][1] = 15.0   # 更高位置
                    segment['position'][2] = 3.0    # 更向前
                else:
                    # 右侧固定点 - 调整到更自然的头部侧面位置
                    segment['position'][0] = 5.0   # 极端远离头部
                    segment['position'][1] = 15.0   # 更高位置
...
def fix_hair_bones_position(model):
    ...
    try:
        ...
        for i, bone in enumerate(sorted(left_hair_bones, key=lambda b: int(b.name.split('_')[2]) if '_' in b.name and len(b.name.split('_')) >= 3 else 0)):
            ...
            target_x = -5.0  # 更远离头部
            target_y = 15.0 + i * 1.2  # 从更高位置开始，每段增加1.2（更极端的向下）
            target_z = 3.0 + i * 0.2  # 更向前，远离眼睛
            ...
            print(f"左侧骨骼 {bone.name} 极端修正后位置: {corrected_pos}")
        ...
        for i, bone in enumerate(sorted(right_hair_bones, key=lambda b: int(b.name.split('_')[2]) if '_' in b.name and len(b.name.split('_')) >= 3 else 0)):
            ...
            print(f"右侧骨骼 {bone.name} 极端修正后位置: {corrected_pos}")
        
        # 更新骨骼矩阵
        model.update_bone()
        
        print("麻花辫骨骼位置已极端修正到完全垂直向下")
    except Exception as e:
